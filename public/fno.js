...
const state = {
  ...
  activeInterval: '1d',
  activeRange: '1y',
  volOverlayMode: 'volintel', // 'off' | 'simple' | 'volintel' (Price Pane Overlay)
  toggles: {
    ema10: true,
    ema20: true,
    ema50: true,
        ...
        <td class="py-2.5 px-3 sticky-col-cell bg-dark-card group-hover:bg-slate-800/60 border-r border-dark-border/40 col-stock">
          <div class="flex flex-col">
            <div class="flex items-center gap-1.5">
              ...
              ${getCircuitBadgeHtml(s.symbol, s)}
            </div>
...
function updateLineStyle(indicatorKey) {
  const colorMap = {
    ...
    volIntelMa: ['setting-color-vol-avg', 'color-vol-avg']
  };
  ...
  if (s) {
    ...
    if ((indicatorKey === 'volAvg' || indicatorKey === 'volIntelMa') && s.volAvg) s.volAvg.applyOptions(opts);
...
function updateSimpleVolSetting(type, checked) {
  if (type === 'vol' || type === 'volume') {
    state.toggles.vol = Boolean(checked);
...
function openLineSettingsModal() {
  ...
  // Sync Volume Display Mode Radios
  const volMode = state.volOverlayMode || (state.toggles.volIntel ? 'volintel' : (state.toggles.volume || state.toggles.vol ? 'simple' : 'off'));
  ...
  // Sync Simple Volume Overlay & 9-SMA Toggles
  ...
  const colorMap = {
    ...
    volIntelMa: 'setting-color-vol-avg'
  };
...
function saveIndicatorPreferences() {
  const prefs = {
    volOverlayMode: state.volOverlayMode || (state.toggles.volIntel ? 'volintel' : (state.toggles.volume || state.toggles.vol ? 'simple' : 'off')),
    toggles: { ...state.toggles },
    colors: { ...state.colors },
...
function applyLoadedIndicatorPreferences(prefs) {
  ...
  if (prefs.volOverlayMode) {
    ...
    state.toggles.volIntel = (prefs.volOverlayMode === 'volintel');
    state.toggles.volume = (prefs.volOverlayMode === 'simple');
    state.toggles.vol = (prefs.volOverlayMode === 'simple');
...
function updateDefaultVolumeBadges() {
  ...
  const viPpBadge = document.getElementById('vi-pp-badge');
  const viBsBadge = document.getElementById('vi-bs-badge');
  const viUdBadge = document.getElementById('vi-ud-badge');
  const viRvolBadge = document.getElementById('vi-rvol-badge');
  const viSignalsBadge = document.getElementById('vi-signals-badge');

  if (mode === 'volintel' && viData?.stats) {
    if (maLabel) maLabel.textContent = `${state.volIntelSettings?.volMaPeriod || 50}-SMA:`;
    if (volBadge && volArr && volArr.length > 0) {
      volBadge.textContent = fmt.volume(volArr[volArr.length - 1]?.value || 0);
    }
    if (volAvgBadge && viData.volMaSeries && viData.volMaSeries.length > 0) {
      volAvgBadge.textContent = fmt.volume(viData.volMaSeries[viData.volMaSeries.length - 1]?.value || 0);
    }
    if (viPpBadge) viPpBadge.textContent = viData.stats.ppCount || 0;
    if (viBsBadge) viBsBadge.textContent = viData.stats.bsCount || 0;
    if (viUdBadge) viUdBadge.textContent = `${viData.stats.latestUD || '--'} (${viData.stats.udStatus || ''})`;
    if (viRvolBadge) viRvolBadge.textContent = `${viData.stats.latestRVol || 1.0}x`;
    ...
    if (viSignalsBadge) {
      if (viData.stats.signals && viData.stats.signals.length > 0) {
        viSignalsBadge.textContent = viData.stats.signals.join(' · ');
        viSignalsBadge.classList.remove('hidden');
      } else {
        viSignalsBadge.classList.add('hidden');
      }
    }
  } else {
    ...
    if (volAvgArr && volAvgArr.length > 0 && volAvgBadge) {
      volAvgBadge.textContent = fmt.volume(volAvgArr[volAvgArr.length - 1]?.value || 0);
    }
    ...
    if (viSignalsBadge) viSignalsBadge.classList.add('hidden');
  }
...
function initNativeCharts() {
  ...
  const volumeSeries = mainChart.addHistogramSeries({
    ...
    priceScaleId: '',
    visible: Boolean(state.toggles.vol)
  });
...
function setupPaneResizers() {
  ...
  // 3. Resizer: Overall Chart Card Bottom Handle
...
function restoreChartModalSize() {
  ...
  if (savedW && savedH) {
    ...
    if (w >= 380 && h >= 320 && w <= window.innerWidth && h <= window.innerHeight) {
...
function initChartModalResizer() {
  ...
  const onDragMove = (e) => {
    ...
    if (dragMode === 'corner' || dragMode === 'right') {
      const newW = Math.round(Math.max(380, Math.min(window.innerWidth * 0.98, startW + deltaX * 2)));
      chartPane.style.width = `${newW}px`;
    }

    if (dragMode === 'corner') {
      const newH = Math.round(Math.max(320, Math.min(window.innerHeight * 0.96, startH + deltaY * 2)));
...
function selectStock(stockOrSymbol, name = '') {
  ...
  updateOnChartCircuitBadge(stockOrSymbol);
  loadStockChart(sym);
...
function openStockChartModal(symbol, name, stockList) {
  ...
  // Load fresh indicator preferences & styles from user settings / main chart
  loadSavedIndicatorPreferences();
  ...
  // Ensure current series visibility and styles match user preferences
  if (state.charts?.series) {
    applyLoadedIndicatorPreferences({
      toggles: state.toggles,
      colors: state.colors,
...
function initIndicatorToggles() {
  const toggleMap = [
    ...
    { id: 'chk-vol', key: 'vol', custom: (checked) => {
      setVolumeOverlayMode(checked ? 'simple' : 'off');
    }},
    ...
    { id: 'chk-vol-intel', key: 'volIntel', custom: (checked) => {
      setVolumeOverlayMode(checked ? 'volintel' : 'off');
    }},
...
async function submitCircuitData() {
  ...
  try {
    ...
    // Re-render table and update current chart badge
